...
class ModuleSummary(BaseModel):
    ...
    # Pipeline run that produced this module; null for manual / legacy rows.
    ...
    published_by: UserActorRef | None = None
    deactivated_by: UserActorRef | None = None
    activated_by: UserActorRef | None = None
    retired_by: UserActorRef | None = None
...
class ModuleEditRequest(BaseModel):
    ...
    Omitted or unchanged ``chatbot_faqs_only``, ``content_domain``, ``domain``, and
    ``estimated_minutes`` do not break the no-op; an explicit different value still
    creates a new draft version. Omitted content fields always version-bump.
    """
    ...
    domain: str | None = Field(
        default=None,
        description=(
            "Catalog domain label for this module version. Omit to copy forward; "
            "send a value to set (normalized to lowercase snake_case)."
        ),
    )
    estimated_minutes: int | None = Field(
        default=None,
        ge=1,
        description="Estimated duration in minutes. Omit to copy forward.",
    )
...
class CardMediaAnchor(BaseModel):
    ...
    strategy: Literal["source_proximity", "hybrid", "llm_draft"] = "hybrid"
