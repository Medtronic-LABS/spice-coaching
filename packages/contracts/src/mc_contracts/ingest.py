"""Ingest API contracts — platform → admin dashboard."""
...
class IngestStartRequest(BaseModel):
    ...
    override_duplicates: list[bool] | None = Field(
        ...
        description=(
            ...
            "re-ingest an already-ingested document in place on the same source_document id"
        ),
    )
...
class IngestStartResponse(BaseModel):
    """Result of ``POST /admin/ingest`` when sources were queued."""
...
class IngestMergeSplitResponse(BaseModel):
    """Result of ``POST /admin/ingest/modules/{module_id}/split-merge``."""

    primary_module_id: uuid.UUID
    secondary_module_id: uuid.UUID
    source_module_id: uuid.UUID | None = None
