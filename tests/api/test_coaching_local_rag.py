"""Coaching local RAG route tests."""
...
async def app(db_session: AsyncSession) -> AsyncIterator[FastAPI]:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr(
            "platform_service.services.coaching_rag_service.CoachingRagService.local_query",
            AsyncMock(
                return_value=CoachingRagResponse(
                    answer="স্থানীয় উত্তর",
                    retrieved_modules=[],
                    source_documents=[],
                    model="qwen-local",
...
class TestCoachingLocalRagRoute:
    async def test_local_rag_query_invokes_service(self, client: AsyncClient) -> None:
        resp = await client.post(
            platform_path("/coaching/local-rag-query"),
            json={"question": "ANC visit steps?"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["answer"] == "স্থানীয় উত্তর"
