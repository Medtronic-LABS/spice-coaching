...
async def _wipe_data_between_tests(db_session: AsyncSession) -> AsyncIterator[None]:
    ...
    await db_session.execute(
        text(
            "TRUNCATE badge_module, badge, module, module_family, "
...
class TestAdminBadges:
    ...
    async def test_list_created_by_filter(
        ...
        for user_id, name in (
            ...
            resp = await client_with_spice.post(
                ...
                json={
                    "name": name,
                    "domain": "rmnch",
                    "image_storage_path": f"badges/{user_id}.png",
                    "module_ids": [],
                },
        ...
        by_multi = await client_with_spice.get(
            platform_path("/admin/badges"),
            params={"created_by": f"{AM_ID},{PO_ID}"},
        )
    ...
    async def test_list_created_by_and_module_title_combined(
        ...
        match = await client_with_spice.post(
            ...
            headers={"X-Test-User-Id": str(AM_ID)},
        )
        ...
        await client_with_spice.post(
            ...
            headers={"X-Test-User-Id": str(PO_ID)},
        )
        assert match.status_code == 201

        combined = await client_with_spice.get(
            platform_path("/admin/badges"),
            params={"created_by": str(AM_ID), "module_title": "Module A"},
        )
...
async def app_with_spice(db_session: AsyncSession) -> AsyncIterator[FastAPI]:
    ...
    async def inject_spice_user(request: Request, call_next):  # type: ignore[no-untyped-def]
        ...
        user_id = int(request.headers.get("X-Test-User-Id", "42"))
        request.state.spice_user = SpiceUserContext.model_validate({"id": user_id, "username": username})
...
class TestAdminBadgesAuditActors:
    async def test_created_by_and_updated_by_from_spice_user(
        ...
        created = create.json()
        badge_id = created["id"]
        assert created["created_by"] == {"id": AM_ID, "name": "Test Area Manager"}
        assert created["updated_by"] == {"id": AM_ID, "name": "Test Area Manager"}
        ...
        assert update.status_code == 200
        updated = update.json()
        assert updated["created_by"] == {"id": AM_ID, "name": "Test Area Manager"}
        assert updated["updated_by"] == {"id": PO_ID, "name": "Test PO"}

        delete = await client_with_spice.delete(
            platform_path(f"/admin/badges/{badge_id}"),
            headers={"X-Test-User-Id": str(SK_ID)},
        )
        assert delete.status_code == 204

        row = (await db_session.execute(select(Badge).where(Badge.id == UUID(badge_id)))).scalar_one()
        assert row.created_by == AM_ID
        assert row.updated_by == SK_ID
