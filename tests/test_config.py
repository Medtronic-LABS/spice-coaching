...
def test_model_defaults_are_gemini_25() -> None:
    ...
    s = Settings()
    # Default provider is "google" (set by the architecture-reset compose).
    assert s.ai_cloud_provider == "google"
    assert s.text_model == "gemini-2.5-flash"
    assert s.vision_model == "gemini-2.5-flash"
    assert s.identification_model == "gemini-2.5-pro"
...
def test_spice_referral_set_parses_destinations_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    # Default is empty (deployer-supplied allowlist); confirm parsing
    # picks up a comma-separated env override and strips whitespace.
    monkeypatch.setenv(
        "SPICE_REFERRAL_DESTINATIONS",
        "Clinic A, Clinic B ,Clinic C",
    )
    s = Settings()
    assert s.spice_referral_set == frozenset({"Clinic A", "Clinic B", "Clinic C"})
    assert "" not in s.spice_referral_set


def test_spice_referral_set_default_is_empty() -> None:
    s = Settings()
    assert s.spice_referral_set == frozenset()
