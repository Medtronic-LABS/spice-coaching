...
class TestHappyPath:
    ...
    async def test_post_publish_enqueue_called_with_module_id(self, db_session: AsyncSession) -> None:
        ...
        StageDOrchestrator._enqueue_post_publish = staticmethod(enqueue_spy)  # type: ignore[assignment]
        ...
        # Verify the module_id passed matches the result.
        called_with = enqueue_spy.call_args.args[0]
        assert called_with == result.module_id
...
# ─── Architecture-reset behaviour ──────────────────────────────────────────


class TestArchitectureResetBehaviour:
    async def test_primary_gap_id_null_when_candidate_has_no_gap_code(self, db_session: AsyncSession) -> None:
        candidate = await _seed_candidate(db_session, behavioural_gap_code=None)
        drafter = _make_card_drafter_mock([_make_card() for _ in range(5)])
    ...
    async def test_primary_gap_id_null_for_legacy_new_gap_proposed_marker(
        self, db_session: AsyncSession
    ) -> None:
        # Some legacy candidates might still carry "new_gap_proposed" as a
        # placeholder. Worker treats it as "no real gap_id".
