...
class TestOutlineEmptyIsAcceptable:
    ...
    async def test_no_heading_markers_anywhere_succeeds_with_warning(
        ...
        with patch(
            ...
            with caplog.at_level("WARNING", logger="platform_service.workers.stage_a_extract"):
                await stage_a.run(
...
async def _fast_recovery_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    backoffs). We patch asyncio.sleep at module level for stage_a_extract so
    only the recovery code's sleeps get neutralised — production would still
    wait the configured durations."""

    async def _no_sleep(_seconds: float) -> None:  # noqa: D401
        return None

    monkeypatch.setattr(
        "platform_service.workers.stage_a_extract.asyncio.sleep",
        _no_sleep,
    )
