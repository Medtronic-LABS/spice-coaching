...
def mock_embed(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    """Patch AIRuntimeClient.embed in the worker's namespace.

    The worker constructs `AIRuntimeClient()` inline; we replace the class
    with a stub whose `.embed(texts)` returns a configurable list of vectors.
    """
    ...
    monkeypatch.setattr("platform_service.workers.embedding_worker.AIRuntimeClient", _StubClient)
    return embed_mock
