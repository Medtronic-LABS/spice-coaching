...
class TestRenderSystemPrompt:
    ...
    def test_omits_case_setup_field_schema(self) -> None:
        settings = get_settings()
        variables = build_quiz_generation_variables(
            module_title="Sample",
            domain="anc",
        ...
        assert "case_setup_field_schema" not in variables

    def test_question_schema_describes_self_contained_stem(self) -> None:
        settings = get_settings()
        variables = build_quiz_generation_variables(
            module_title="Sample",
            domain="anc",
        ...
        schema = variables["question_field_schema"].lower()
        assert "self-contained" in schema
        assert "patient case" in schema
...
async def _seed_module(
    ...
    *,
    ...
    fam = ModuleFamily(module_code=f"f-{uuid4().hex[:8]}", tenant_id=1)
    session.add(fam)
    await session.flush()
...
class TestHappyPath:
    ...
    async def test_writes_questions_with_module_fk(
        ...
        # case_setup is no longer populated on new writes.
        assert all(r.case_setup_localized is None for r in rows)
