...
class TestMigrationSourceContents:
    ...
    def migration_source(self) -> str:
        path = Path(__file__).parents[2] / "infra" / "alembic" / "versions" / "0007_architecture_reset.py"
        assert path.exists(), f"migration file not found: {path}"
        return path.read_text()

    def test_legacy_lifecycle_values_coerced_to_retired(self, migration_source: str) -> None:
        # The exact SQL: UPDATE module SET lifecycle_status = 'retired' WHERE
        # lifecycle_status IN ('deprecated', 'archived')
        assert "lifecycle_status = 'retired'" in migration_source
        assert "'deprecated'" in migration_source
        assert "'archived'" in migration_source

    def test_in_review_and_approved_coerced_to_draft(self, migration_source: str) -> None:
        # The pre-publish states get rolled back to 'draft' since the
        # auto-publish flow doesn't transit through them.
        assert "lifecycle_status = 'draft'" in migration_source
        assert "'in_review'" in migration_source
        assert "'approved'" in migration_source

    def test_quality_flags_columns_added(self, migration_source: str) -> None:
        # Both module + module_candidate_draft get quality_flags_jsonb.
        assert migration_source.count("quality_flags_jsonb") >= 2

    def test_module_quiz_question_table_truncated_before_fk(self, migration_source: str) -> None:
        """The migration TRUNCATEs `module_quiz_question` before adding the
        NOT NULL module_id column, since pre-existing orphan rows had no
        parent module to backfill. Without the truncate, the ALTER fails."""
        # Find the TRUNCATE before the module_id ADD COLUMN.
        truncate_idx = migration_source.find("DELETE FROM module_quiz_question")
        if truncate_idx == -1:
            truncate_idx = migration_source.find("TRUNCATE module_quiz_question")
        add_module_id_idx = migration_source.find('"module_id"')
        assert truncate_idx >= 0
        assert add_module_id_idx >= 0
        assert truncate_idx < add_module_id_idx, (
            "Migration must clear `module_quiz_question` before adding the "
            "NOT NULL module_id FK; otherwise pre-existing rows with no "
            "parent module would block the ALTER."
        )
