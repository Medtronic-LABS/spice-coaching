...
class TestPromptGapContextRemoved:
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"supervisor_update"},
            {"spice_digital"},
            {"official_training", "supervisor_update"},
        ],
    )
    def test_system_prompt_does_not_reference_gap_registry(self, authority_kinds: set[str]) -> None:
        prompt = render_system_prompt(authority_kinds)
    ...
    def test_system_prompt_does_not_request_gap_code_field(self) -> None:
        prompt = render_system_prompt({"official_training"})
    ...
    def test_template_version_at_least_5(self) -> None:
        ...
        assert MODULE_IDENTIFIER_TEMPLATE_VERSION >= 5
...
class TestAuthorityKindBranching:
    def test_official_training_branch_is_default(self) -> None:
        prompt = render_system_prompt(set())
        assert "OFFICIAL TRAINING" in prompt or "initial_training" in prompt
    ...
    def test_spice_digital_branch(self) -> None:
        prompt = render_system_prompt({"spice_digital"})
        assert "SPICE DIGITAL" in prompt
        assert "digital_proficiency" in prompt

    def test_mixed_authority_picks_mixed_branch(self) -> None:
        prompt = render_system_prompt({"supervisor_update", "official_training"})
...
class TestIdentifyHappyPath:
    ...
    async def test_returns_validated_candidates_dict_payload(self) -> None:
        ...
        result = await identifier.identify(
            authority_kinds={"official_training"},
            already_published_modules=[],
            document_outlines=[],
            page_corpus=[],
            valid_block_ids={b1},
