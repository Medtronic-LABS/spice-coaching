...
def test_create_object_store_gcs() -> None:
    ...
    store = create_object_store(settings=settings)
    assert isinstance(store, S3ObjectStore)
    assert store.bucket_name == "my-gcs-bucket"
    assert store._backend == "gcs"
