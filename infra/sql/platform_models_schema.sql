...
CREATE INDEX IF NOT EXISTS ix_module_tenant_id
  ON module (tenant_id);
CREATE INDEX IF NOT EXISTS ix_module_created_by
  ON module (created_by);
CREATE INDEX IF NOT EXISTS ix_module_published_by
  ON module (published_by);
CREATE INDEX IF NOT EXISTS ix_module_deactivated_by
  ON module (deactivated_by);
CREATE INDEX IF NOT EXISTS ix_module_activated_by
  ON module (activated_by);
...
-- users.role text → role table + users.role_id in 0085)
-- AM → PO → SK; role catalog seeded with AREA_MANAGER | PO | SHASTIYA_KORMI | SUPER_ADMIN
-- SUPER_ADMIN is a root role (parent_id NULL) like AREA_MANAGER; not creatable via admin API
-- SUPER_ADMIN may have district_id NULL (auth auto-provision); other roles require a district
-- ──────────────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS api_route (
  ...
  CONSTRAINT uq_api_route_path_template UNIQUE (path_template)
);

CREATE TABLE IF NOT EXISTS role_route_access (
  role_id bigint NOT NULL REFERENCES role(id) ON DELETE CASCADE,
  api_route_id bigint NOT NULL REFERENCES api_route(id) ON DELETE CASCADE,
  PRIMARY KEY (role_id, api_route_id)
);

CREATE INDEX IF NOT EXISTS ix_role_route_access_role_id ON role_route_access (role_id);
...
CREATE TABLE IF NOT EXISTS users (
  ...
  district_id bigint NULL REFERENCES district(id) ON DELETE CASCADE,
  tenant_id bigint NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  created_by bigint NULL,
...
CREATE TABLE IF NOT EXISTS config_threshold_change (
  id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  tenant_id bigint NOT NULL,
  config_threshold_id integer NOT NULL REFERENCES config_threshold(id) ON DELETE CASCADE,
  key text NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_config_threshold_change_tenant_key_updated
  ON config_threshold_change (tenant_id, key, updated_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS ix_config_threshold_change_config_threshold_id
  ON config_threshold_change (config_threshold_id);
...
INSERT INTO config_threshold_change (tenant_id, config_threshold_id, key, previous_value_json, current_value_json, version, updated_by)
SELECT
  ct.tenant_id,
  ct.id,
  ct.key,
...
JOIN (VALUES
  ('learning_points_module_delivered', '5'::jsonb),
  ('learning_points_module_card_viewed', '10'::jsonb),
  ('learning_points_module_quiz_attempted_base', '15'::jsonb),
  ('learning_points_module_quiz_score_multiplier', '15'::jsonb),
  ('learning_points_module_completed', '20'::jsonb),
  ('learning_points_spice_action_observed', '3'::jsonb),
  ('quiz_reattempt_validity_days', '7'::jsonb)
) AS v(key, value_json) ON ct.key = v.key
WHERE NOT EXISTS (
  SELECT 1 FROM config_threshold_change AS ctc WHERE ctc.config_threshold_id = ct.id
);
