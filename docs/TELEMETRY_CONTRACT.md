...
**Status:** Drafted on `module-generation-changes` ahead of telemetry-branch convergence. The body describes the **post-merge target state** — the field tables and enum values reflect what the SDK should send once the small contract diffs (covered in this doc) land on the telemetry branch. The only code-side change carried with this doc is the `TriggerType` widening in `enums.py`. The `TelemetryEvent` fields `module_id`, `card_family_id`, `quiz_family_id` exist on the telemetry branch but not yet here; the doc describes the unified contract that emerges once both branches reach `main`.
...
| `quiz_family_id` | string (UUID) \| null | `module_quiz_question.question_family_id`. Stable across module versions. |
...
batch arrives
  ...
  └─ for each first-seen event:
       ...
       └─ if event_type ∈ {module_delivered, module_card_viewed,
                           module_quiz_attempted, module_completed}
...
For CHWs identified by batch-level `chw_id`, the same `process_module_event_task` worker inserts rows into **`chw_learning_point_event`** (one row per scored telemetry `id`, with a **`points`** delta) when it accepts these `event_type` values: **`module_delivered`**, **`module_card_viewed`**, **`module_quiz_attempted`**, **`module_completed`**, and **`spice_action_observed`**. The CHW's total is **`SUM(points)`** for that `chw_id` (indexed). Point amounts are configurable in platform settings (`learning_points_*`); **`module_quiz_attempted`** adds a score-based bonus from `quiz_score_pct` (0.0–1.0).
...
| **`module_completed`** | v3.3 module marked complete after passing quiz | coaching_events + `process_module_event_task` (completion + **learning points**) |
...
```json
{
  ...
  "payload_json": {
    "answers": [
      {"quiz_family_id": "...", "selected_options": [1, 2], "correct": true},
      {"quiz_family_id": "...", "selected_options": [0],    "correct": false}
    ]
  },
  "event_date": "2026-04-28",
...
Fired after `module_quiz_attempted` if the score passes the threshold (default 0.7; per-module override possible). The `process_module_event_task` worker writes `chw_module_completion` and updates `chw_behavioural_gap_state`.

```json
{
  "id": "ab3e6f2a-4d8a-6b9c-ad7e-3a5b1e4f6d8c",
  "event_schema_version": 1,
  "event_family": "coaching",
