"""Truncation-tolerant JSON salvage helpers — shared by Stage 2
identifier and consolidator.
...
Underscore-prefixed re-exports (`_salvage_truncated_array`,
`_extract_inner_array`) are kept for one release as compatibility shims
so callers that imported them directly from `module_identifier` /
`module_consolidator` don't break.
"""
