"""Stage 2 — module-identification prompt (per-authority-kind variants).
...
source_document.authority_kind:

- official_training / clinical_guideline → emit initial_training candidates
  for comprehensive training manuals
- supervisor_update → emit content_update candidates with previous/current/
  rationale fields populated
- spice_digital domain → emit digital_proficiency candidates
...
# v5: added rule to exclude annexures (forms, checklists, reporting
# templates, reference tables) from module proposals. Surfaced on the
# ASHA NCDs Hindi run where the identifier proposed two modules from
# Annexure 1-4 pages (Family Folder form, CBAC checklist, services-by-
# facility-level table). Cache entries from v4 will not be reused.
MODULE_IDENTIFIER_TEMPLATE_VERSION = 5
...
Authority-kind branching:
{authority_branch_instructions}
...
and content blocks instead of full UUIDs. You will see headers like:
    ...
    ### source_document_id=d1 authority_kind=...
...
_BRANCH_OFFICIAL_TRAINING = """\
This corpus contains OFFICIAL TRAINING / CLINICAL GUIDELINE content (current
correct protocol). Emit modules with proposed_module_type = "initial_training".
...
_BRANCH_SPICE_DIGITAL = """\
This corpus contains SPICE DIGITAL workflow content. Emit modules with
...
_BRANCH_MIXED = """\
This corpus contains MIXED authority kinds. For each candidate, choose
...
- if all cited blocks are from spice_digital-domain documents → digital_proficiency
...
def _branch_instructions_for(authority_kinds: set[str]) -> str:
    """Pick the correct branch text for the workspace's authority mix."""
    if not authority_kinds:
        return _BRANCH_OFFICIAL_TRAINING  # default
    if "supervisor_update" in authority_kinds and len(authority_kinds) > 1:
        return _BRANCH_MIXED
    if authority_kinds == {"supervisor_update"}:
        return _BRANCH_SUPERVISOR_UPDATE
    if authority_kinds == {"spice_digital"}:
        return _BRANCH_SPICE_DIGITAL
    return _BRANCH_OFFICIAL_TRAINING


def render_system_prompt(authority_kinds: set[str]) -> str:
    """Render the system prompt with the authority-kind-specific branch."""
    return SYSTEM_PROMPT.format(
        authority_branch_instructions=_branch_instructions_for(authority_kinds),
    ...
    *,
    ...
    for doc in page_corpus:
        body_lines.append(
            ...
            f"authority_kind={doc.get('authority_kind', 'unknown')} "
