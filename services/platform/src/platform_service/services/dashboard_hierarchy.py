...
def actor_view_role(view: DashboardActorView) -> str:
    """Map ``view=po|sk`` to the hierarchy role string stored on org users."""
    if view == DashboardActorView.PO:
        return HierarchyRole.PO.value
    if view == DashboardActorView.SK:
        return HierarchyRole.SHASTIYA_KORMI.value
    raise ValueError(f"unsupported dashboard actor view: {view!r}")


async def filter_chw_ids_by_role(
    session: AsyncSession,
    *,
    tenant_id: int,
    ...
    """Narrow ``chw_ids`` to users with ``role``; never widens visibility.

    When ``chw_ids`` is ``None`` (unrestricted), returns every tenant user id with
    that role. Missing ids in the org map are excluded.
    """
    by_id = await org_user_index(session, tenant_id=tenant_id)
    if chw_ids is None:
        return frozenset(u.id for u in by_id.values() if u.role == role)
    return frozenset(uid for uid in chw_ids if (user := by_id.get(uid)) is not None and user.role == role)
...
async def apply_document_usage_filters(
    ...
    *,
    ...
    geo_user_ids = await resolve_users_by_geography_ids(
        session,
        tenant_id=tenant_id,
...
def _casefolded_name_set(names: list[str] | None) -> set[str] | None:
    if not names:
        return None
    needles = {name.strip().casefold() for name in names if name.strip()}
    return needles or None


async def resolve_users_by_geography_names(
    session: AsyncSession,
    *,
    tenant_id: int,
    ...
    """Return user ids matching optional division/district/upazila name filters.

    Multiple values within a dimension match with OR; dimensions AND together.
    Returns None when all inputs unset; otherwise a frozenset (possibly empty).
    """
    div_needles = _casefolded_name_set(divisions)
    dist_needles = _casefolded_name_set(districts)
    upa_needles = _casefolded_name_set(upazilas)
    if div_needles is None and dist_needles is None and upa_needles is None:
        return None
    ...
    if div_needles is not None:
        division_ids = {
            user.id
            for user in by_id.values()
            if user.division is not None and user.division.casefold() in div_needles
        }
        _intersect(division_ids)

    if dist_needles is not None:
        district_ids = {
            user.id
            for user in by_id.values()
            if user.district is not None and user.district.casefold() in dist_needles
        }
        _intersect(district_ids)

    if upa_needles is not None:
        upazila_ids = {
            user.id
            for user in by_id.values()
            if any(name.casefold() in upa_needles for name in user.upazila_names)
        }
        _intersect(upazila_ids)

    return frozenset(candidates or set())


async def resolve_users_by_geography_ids(
    session: AsyncSession,
    *,
    tenant_id: int,
    division_ids: list[int] | None = None,
    ...
    """Return user ids matching optional division/district/upazila id filters.

    Multiple values within a dimension match with OR; dimensions AND together.
    Returns None when all inputs unset; otherwise a frozenset (possibly empty).
    """
    div_ids = set(division_ids) if division_ids else None
    dist_ids = set(district_ids) if district_ids else None
    upa_ids = set(upazila_ids) if upazila_ids else None
    if div_ids is None and dist_ids is None and upa_ids is None:
        return None
    ...
    if div_ids is not None:
        division_user_ids = {
            user.id for user in by_id.values() if user.division_id is not None and user.division_id in div_ids
        }
        _intersect(division_user_ids)

    if dist_ids is not None:
        district_user_ids = {user.id for user in by_id.values() if user.district_id in dist_ids}
        _intersect(district_user_ids)

    if upa_ids is not None:
        upazila_user_ids = {user.id for user in by_id.values() if user.upazila_ids & upa_ids}
        _intersect(upazila_user_ids)
...
async def resolve_geography_chw_ids(
    ...
    *,
    ...
    """Return user ids matching optional division/district/upazila id filters, or None when unset."""
