...
Mirrored by the PostgreSQL ``enforce_users_hierarchy`` trigger
(migrations 0059 / 0066 / 0085 / 0086 / 0089). Admin create APIs only allow
``HIERARCHY_ROLES`` (AM/PO/SK); ``SUPER_ADMIN`` is trigger-supported as a
root role (``parent_id`` NULL, ``district_id`` may be NULL) for out-of-band
and auth-path auto-provisioning.
"""
