...
class BadgeService:
    ...
    async def list(
        ...
        *,
        ...
        actor_ids = [uid for b in badges for uid in (b.created_by, b.updated_by) if uid is not None]
    ...
    async def soft_delete(self, badge_id: UUID, *, actor: int | None = None) -> None:
        badge = await self._repo.get_active(badge_id)
        if badge is None:
            raise AppError(
                ErrorCode.BADGE_NOT_FOUND.value,
    ...
    async def _to_response(self, badge: Badge) -> BadgeResponse:
        ...
        actor_ids = [uid for uid in (badge.created_by, badge.updated_by) if uid is not None]
        users_by_id = await HierarchyRepository(self._session).get_users_by_ids(actor_ids)
        return self._badge_response(badge, modules_by_badge.get(badge.id, []), users_by_id=users_by_id)
    ...
    def _badge_response(
        ...
        *,
        ...
        return BadgeResponse(
            ...
            created_by=to_user_actor_ref(badge.created_by, users_by_id),
            updated_by=to_user_actor_ref(badge.updated_by, users_by_id),
        )
