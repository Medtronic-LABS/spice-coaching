...
class DraftPipeline:
    ...
    async def draft_and_validate_cards(
        ...
        *,
        ...
        image_catalog = await self._load_image_catalog(candidate_dict)
    ...
    async def _load_image_catalog(
        self,
        candidate_dict: dict[str, Any],
    ) -> list[ImageCatalogEntry]:
        """Load usable source_image rows and build the short-id catalog.

        Returns an empty list when image assignment is disabled or no source
        documents are attached to the candidate.
        """
        settings = get_settings()
        if not settings.ingest_card_image_assignment_enabled:
            return []
        source_document_ids = self.extract_source_document_ids(candidate_dict)
        if not source_document_ids:
            return []
        repo = SourceRepository(self._session)
        images = await repo.list_images_for_documents(source_document_ids)
        catalog = build_image_catalog(images)
        logger.debug(
            "draft pipeline image catalog candidate_docs=%d total_images=%d usable=%d",
    ...
    async def enqueue_post_publish(
        ...
        *,
        ...
        publish (see ``module_publish_enrichment``): parallel card+module
        metadata when ``publish_enrichment_parallel_metadata_enabled`` is on,
        then embedding. Creates ``ingestion_run_step`` rows so ingest polling
        can track quiz/gap/trigger progress.
        """
