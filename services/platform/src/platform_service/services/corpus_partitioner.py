...
from platform_service.workers.extractors.content_block_parser import estimate_token_count
...
def chunk_by_token_budget(
    ...
    for doc in page_corpus:
        ...
        authority = doc.get("authority_kind")
...
def _trigrams(s: str) -> set[str]:
    """Character trigrams of a normalised string (for similarity scoring)."""
    s = _normalise_title(s)
    if len(s) < 3:
...
def dedup_and_flag_cross_chunk(
    ...
    for chunk_id, candidates in chunked_candidates:
        for cand in candidates:
            key = _normalise_title(cand.get("proposed_title", ""))
            if not key:
    ...
    for i, a in enumerate(deduped):
        ...
        for b in deduped[i + 1 :]:
            ...
            if a_chunks & b_chunks:
                continue
            sim = _trigram_similarity(a_title, b.get("proposed_title", ""))
            if sim >= threshold:
