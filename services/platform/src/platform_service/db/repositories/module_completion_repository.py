...
class ModuleCompletionRepository:
    ...
    async def mark_completed(
        ...
        *,
        ...
        Used when the SDK sends a separate MODULE_COMPLETED event after the
        quiz was already scored. No-op if the completion row doesn't exist
        (the quiz attempt should have created it; if it didn't, we skip
        rather than fabricate a row with no score data).
        """
        comp = await self.get(chw_id=chw_id, module_family_id=module_family_id)
        if comp is None:
            return None
