...
class ModuleDrafterRepository:
    ...
    async def get_or_create_module_family(
        self, *, proposed_title: str, created_by: int | None = None, tenant_id: int = DEFAULT_TENANT_ID
    ) -> ModuleFamily:
    ...
    async def create_review_pending_in_family(
        self,
        *,
        family: ModuleFamily,
        matched: Module,
        ...
        """New ``review_pending`` module in ``family`` (dual-path merge staging).

        Dual-path primary/secondary each live in their own family, distinct
        from the matched tip. Does not retire the matched tip, set
        ``supersedes_module_id``, or rewrite any family's
        ``current_published_module_id`` — those happen on admin override-merge.
        Each row gets its own primary behavioural gap. Secondary rows carry
        merge lineage flags.
        """
        version = await self._next_version(family.id)
        ...
        if not primary_text(title_localized):
            raise ValueError(
                f"create_review_pending_in_family: no usable title (proposed_title={proposed_title!r})"
            )
