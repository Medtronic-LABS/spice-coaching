...
class LlmCallCache(TenantMixin, Base):
    __tablename__ = "llm_call_cache"
    __table_args__ = (UniqueConstraint("tenant_id", "input_hash", name="uq_llm_call_cache_tenant_hash"),)

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    # Hash of (prompt + input payload + model). Unique per tenant so duplicate
    # inputs are deduplicated within a tenant.
