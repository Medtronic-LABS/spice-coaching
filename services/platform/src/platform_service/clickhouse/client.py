...
_DIGITIAL_PROFICIENCY_EVENT_COLUMNS = [
    "id",
    "event_schema_version",
    "sdk_version",
    "session_id",
...
class ClickHouseClient:
    ...
    async def insert_digital_proficiency_events(self, rows: list[list[Any]]) -> None:
        """Insert rows into digital_proficiency_events table.

        Args:
            rows: Each row is a list of values in ``_DIGITIAL_PROFICIENCY_EVENT_COLUMNS`` order.
        """
        if not rows:
            return
