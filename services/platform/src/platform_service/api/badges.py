...
async def list_badges(
    ...
    created_by: list[str] | None = Query(
        None,
        description=(
            "Optional filter; exact match on badge.created_by hierarchy user id. "
            "Repeat and/or comma-separate (e.g. created_by=101&created_by=102 "
            "or created_by=101,102)."
        ),
    ),
    ...
    return await BadgeService(session).list(
        domain=domain,
        created_by=normalize_int_csv_query_values(created_by, param_name="created_by"),
