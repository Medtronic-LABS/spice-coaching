    ...
    return IngestStartResponse(
        ...
        sources=[
            IngestStartAcceptedSource(
                ...
                ingested_by=to_user_actor_ref(source.ingested_by_user_id, users_by_id),
            )
...
async def override_ingest_merge(
    ...
    """Promote secondary in its own family; retire primary and matched source."""
    result = await IngestMergeOverrideService(db).override(
        module_id,
...
@router.post(
    "/ingest/modules/{module_id}/split-merge",
    response_model=IngestMergeSplitResponse,
)
async def split_ingest_merge(
    request: Request,
    module_id: uuid.UUID,
    db: AsyncSession = Depends(get_db),
) -> IngestMergeSplitResponse:
    """Keep primary in its own family as draft; retire secondary."""
    result = await IngestMergeSplitService(db).split(
        module_id,
        retired_by_user_id=resolve_spice_user_id(request),
    )
    return IngestMergeSplitResponse(
        primary_module_id=result.primary_module_id,
        secondary_module_id=result.secondary_module_id,
        source_module_id=result.source_module_id,
        primary_lifecycle_status=result.primary_lifecycle_status,
        secondary_lifecycle_status=result.secondary_lifecycle_status,
    )
