...
def estimate_token_count(text: str) -> int:
    """Rough token estimate for LLM-budget decisions.

    Uses 4 chars per token as a coarse approximation matching Gemini's
    ballpark (English) and slightly under for Bangla (where one Bangla
    codepoint is often one token). Good enough for the corpus-vs-section
    decision; not meant for precise billing.
    """
    if not text:
        return 0
